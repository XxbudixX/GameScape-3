...
function setLoggedIn(status, username, avatarSeed) {
    ...
    if (status && username) {
        ...
        awaitSendMapPresenceThenConnect();
    }
}

function awaitSendMapPresenceThenConnect() {
...
async function loadLivePlayers() {
    try {
        ...
        const cleaned = (data.players || [])
            .map(p => ({ ...p, lat: Number(p.lat), lng: Number(p.lng), mapVisible: true }))
