...
function initChat() {
    ...
    function appendMessageBubble(msg, stacked = false) {
        ...
        const senderName = isSelf ? currentUsername : msg.from;
        const avatarSeed = isSelf
            ? (msg.avatar_seed || currentUserAvatar || currentUsername)
            : (msg.avatar_seed || msg.from);
        ...
        const replyHtml = msg.reply_to_id
            ? `<div class="reply-context${replyClass}">↳ ${escapeHtml(replyFrom || 'Message')}: ${escapeHtml(msg.reply_text || '')}</div>`
            : '';

        const editedHtml = msg.edited_at ? '<span class="edited-label">edited</span>' : '';
        const actionsHtml = (isSelf && !msg.deleted && msg.id)
            ? `<div class="message-actions"><button data-chat-action="reply">Reply</button><button data-chat-action="edit">Edit</button><button data-chat-action="delete">Delete</button></div>`
            : `<div class="message-actions"><button data-chat-action="reply">Reply</button></div>`;

        const div = document.createElement('div');
        div.className = `message-row ${isSelf ? 'self' : ''} ${stacked ? 'stacked' : ''} ${msg.deleted ? 'deleted' : ''}`;
        div.dataset.messageId = msg.id || '';
        div.innerHTML = `
            <div class="message-avatar">${stacked ? '' : `<img src="${avatarUrl(avatarSeed)}" alt="">`}</div>
            <div class="message-main">
            ${stacked ? '' : `<div class="message-head"><span class="sender">${escapeHtml(senderName)}</span><span class="time">${escapeHtml(msg.time || '')}</span></div>`}
            ${replyHtml}
            <div class="message-text">${escapeHtml(msg.deleted ? '[deleted]' : msg.text)} ${editedHtml}</div>
            </div>
            ${actionsHtml}
        `;
